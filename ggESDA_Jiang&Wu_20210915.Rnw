...
\begin{figure}[h]
\centering
<<box_centerRange, echo=FALSE, fig=TRUE, height=6, width=6>>=
...
@
\caption{\label{fig:box_centerRange} Box plot and Center-Range plot}
\end{figure}
...
\begin{figure}[h]
\centering
<<minmax, echo=FALSE, fig=TRUE, height=6, width=8>>=
plotLocal <- NULL
plotGlobal <- NULL
for(i in 1:4){
...
a <- ggarrange(plotlist = plotLocal, nrow = 1, ncol = 4, labels = "(a)")
b <- ggarrange(plotlist = plotGlobal, nrow = 1, ncol = 4, labels = "(b)")
...
\begin{figure}[h]
\centering
<<hist, echo=FALSE, fig=TRUE, height=6, width=8>>=
a <- ggInterval_hist(facedata, plotAll = T) +
  theme_hc()+
  scale_y_continuous(breaks = c(0, 0.1, 0.2))+
  labs(title = "Equidistant-bin histogram")


b<-ggInterval_hist(facedata, plotAll = T,
                   method="unequal-bin") +
  theme_hc()+
  scale_y_continuous(breaks = c(0, 0.2, 0.4))+
  labs(title = "Non-equidistant-bin histogram")
...
\begin{figure}[h]
\centering
<<index, echo=FALSE, fig=TRUE, height=6, width=6>>=
m <- mean(facedata$AD)
p2 <- ggInterval_index(facedata, aes(x = AD, fill = Concepts))+
  theme_bw() +
...
p1 <- ggInterval_index(facedata, aes(AD)) +
  theme_bw()+
  scale_y_continuous(breaks = 1:27,
                     labels = rownames(facedata))+
  labs(title = "", y = "")+
...
p3 <- ggInterval_indexImage(facedata, aes(x = AD))+
        coord_flip()+
  theme_bw()+
  labs(title = "")+
  theme(axis.text.y = element_text(size = 6),
...
p4 <- ggInterval_indexImage(facedata, aes(x = AD),
                            full_strip  = T)+
  coord_flip()+
  theme_bw()+
  labs(title = "")+
...
\begin{figure}[h]
\centering
\includegraphics[]{ggESDA_Jiang&Wu_20210915-PCA.pdf
}
