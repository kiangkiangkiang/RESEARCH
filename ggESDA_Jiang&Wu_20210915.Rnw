...
\begin{figure}[htbp]
    \centering
    \scalebox{0.9}{
<<box_centerRange_before, echo=FALSE, fig=TRUE, height=6, width=8>>=
set.seed(20211216)
...
\begin{figure}[htbp]
    ...
    \centering
    \scalebox{0.9}{
<<box_centerRange_after, echo=FALSE, fig=TRUE, height=6, width=8>>=
...
ggInterval_hist(this.data, plotAll = T, bins = 15) +
  theme_hc()+
  scale_y_continuous(breaks = c(0, 0.035, 0.07))+
  labs(title = "")
@
                }}
                &
...
ggInterval_2Dhist(facedata, aes(x = AD, 
                                y = BC, 
                                col = "grey30"),
                  xBins = 25,
                  yBins = 25) + 
...
ggInterval_2DhistMatrix(facedata, aes(col = "grey50"),
                  xBins = 10,
                  yBins = 10,
                  removeZero = T,
                  addFreq = F) + 
...
Above all, take the Environmental questionnaire dataset for example, and a typical multivariate radar plot without modal multi-valued variables is shown in Figure \ref{fig:radar_typical} by the code

<<radar_typicalEx, echo=TRUE, eval=FALSE>>=
Environment.i <- Environment[, 5:17] #take interval-valued
left.fig <- ggInterval_radar(Environment.i, 
                 plotPartial = 2,
                 showLegend = F,
                 base_circle = T,
                 base_lty = 2,
...
right.fig <- ggInterval_radar(Environment.n, 
                 plotPartial = 2,
                 showLegend = F,
                 base_circle = F,
                 base_lty = 1,
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{radar_typical.pdf} 
\caption{\label{fig:radar_typical} Typical radar plot with interval-valued variables.}
\end{figure}

Use the color to represent the interval area, and it can clearly present the behavior of the particular concept in each variable. The figure in the left panel shows the relative position of the interval by the percentage circle, whereas the right one removes it and adds the frequency in the plot.

Secondly, the kind of radar plot is generalized to visualize not only the interval-valued data but the modal multi-valued, which we design a cuboid to represent, and the height will correspond to the probability of the factor in the variable, as Figure \ref{fig:radar_3obs} shown. The technique merges the distinct variable's type in one plot and helps the researcher to compare with concepts easily under multiple variables. 

\begin{figure}[htbp]
\centering
\includegraphics[trim=0cm 3cm 0cm 3cm,width=1\textwidth, clip]{radar_3obs.pdf} 
\caption{\label{fig:radar_3obs} Radar}
\end{figure}
...
we can get the Figure \ref{fig:radar}. It's worth mentioning that whenever the users determine the option \code{plotPartial} with multiple observations and set \code{inOneFig = TRUE} which means if pool observations into the same figure, the mechanism in the function will automatically classify by color. In this case, the two visualized concepts are represented by blue and red color, and the modal multi-valued variables are stacked by the same factor. On the contrast, the interval-valued variables display the graph in two ways. The left panel of the figure shows a typical area-presenting way, while the right one use rectangle to show the interval. These implement can be completed by setting the option \code{type} with \code{"default"} and \code{"rect"} respectively.

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{ggESDA_Jiang&Wu_20210915-radar.pdf} 
\caption{\label{fig:radar} Visualize three distinct observations by the radar plot containing interval-valued and modal multi-valued variables.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{radar_2datasets.pdf} 
