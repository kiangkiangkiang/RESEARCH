...
\begin{figure}[htbp]	
  		\centering	 		
 	 		\includegraphics[width=0.8\textwidth]{doc/ggESDA_Jiang&Wu_20210915-TrendFig.pdf} 
...
\begin{figure}[htbp]	
  		\centering	 		
 	 		\includegraphics[width=0.8\textwidth]{doc/ggESDA_Jiang&Wu_20210915-subjectFig.pdf} 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.6\textwidth]{doc/ggESDA_Jiang&Wu_20210915-compare.pdf} 
...
\begin{table}[h]
  ...
  \resizebox{\textwidth}{!}{
    ...
    RSDA  & Rojas \emph{et al}. (2015) & R(4.1.0)     & \code{classic.to.sym}     & -     & 1     & -     & 3     & \textbf{2}    & \textbf{1} \\
    symbolicDA & Dudek \emph{et al}. (2013) & R(4.1.0)     &    -   & -     & -     & \textbf{2}    & 2     & 1     & \textbf{1} \\
    HistDAWass & Irpino (2015) & R(4.1.0)     & \code{data2hist}     & 4     & -     & -     & 3     & 1     & \textbf{1} \\
    
    MAINT.Data & Silva \&  Brito (2011) & R(4.1.0)     &   -    & -     & -     & -     & \textbf{7}    & -     & \textbf{1} \\\hline
    iRegression & Neto \emph{et al}. (2011) & R(4.1.0)     &   -    & -     & -     & -     & 1     & -     & - \\
...
\centering
\scalebox{.7}{
\includegraphics[trim=0cm 1cm 0cm 1cm,width=0.4\textwidth,clip]{doc/face.pdf}
}
...
<<mmplotEx, echo=TRUE, fig=F, eval=F, eval=F>>=
...
\begin{figure}[htbp]
    \centering
    \includegraphics[trim=5cm 0.5cm 6cm 0.5cm,angle=-90,width=1\textwidth, clip]{doc/mmplot.pdf} 
...
\begin{figure}[htbp]
    ...
    \begin{tabular}{c c c c}
        \subfloat[Index plot]{
        \label{fig:index}
\includegraphics[trim=0.5cm 0.5cm 0cm 0.5cm,width=0.2\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-index.pdf} 
                }
                &
        ...
        \label{fig:index_bar}
        \includegraphics[trim=0.5cm 0.5cm 0.5cm 0.5cm,width=0.2\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-index_bar.pdf} 
        }
        &
                ...
                \label{fig:image}
                \includegraphics[trim=0.5cm 0.5cm 0cm 0.5cm,width=0.2\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-index_image1.pdf} 
        ...
        \includegraphics[trim=0.5cm 0.5cm 0.5cm 0.5cm,width=0.2\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-index_image2.pdf} 
}
        \end{tabular}
...
<<indexImageEx, echo=TRUE, fig=F, eval=F>>=
#Column Condition
...
\begin{figure}[htbp]
    \centering 
    \includegraphics[width=0.85\textwidth,clip]{doc/indexImage_2case.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-box_centerRange_before.pdf}
...
\begin{figure}[htbp]
    ...
    \centering
\includegraphics[width=0.85\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-box_centerRange_after.pdf}
...
\begin{figure}[htbp]
    ...
    \begin{tabular}{c c}
        ...
        \includegraphics[width=0.5\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-hist_equal.pdf}
}
                &
        \subfloat[Non-equidistant-bin histogram]{
        \includegraphics[width=0.5\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-hist_unequal.pdf}
}
        \end{tabular}
...
<<scaMatrixEx, echo=TRUE, eval=FALSE>>=
...
\includegraphics[width=0.5\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-scaMatrix.pdf}

\caption{\label{fig:scaMatrix} Scatter matrix}
\end{figure}


In Figure \ref{fig:scaMatrix}, we use grayscale to represent the concepts and the rectangle to display the intersection of two interval-valued observations. The relationship among this graph is clearly depicted. The variables AH and DH are an obvious positive correlation, whereas AD and AH seems to be an exponential relation. Of course, there is an easy way for scatter plot of two particular variables using the function \code{ggInterval_scatter}. With the technique, we combine the dimension reduction methods that \pkg{RSDA} provides, and implement the maximum covering area rectangle (MCAR) \cite{cazes1997extension} using the code:
...
\begin{figure}[htbp]
    ...
    \begin{tabular}{c c}
        ...
        \label{fig:mcar}
\includegraphics[width=0.4\textwidth]{doc/ggESDA_Jiang&Wu_20210915-PCA_MCAR.pdf
}
                }
                &
        \subfloat[Vertice-PCA (Polytope)]{
        \label{fig:poly}
        \includegraphics[width=0.4\textwidth]{doc/ggESDA_Jiang&Wu_20210915-PCA_Polytope.pdf
}
        }
        \end{tabular}
...
Nevertheless, the main drawback of MCAR is that the interval objects are oversized in the DR subspace with respect to the real objects in $\mathbb{R}^p$, hence, the polytopes representation \cite{le:2012} for interval data was proposed. In \pkg{ggESDA}, we have also developed the graphic function for them using 
...
<<hist_2d_Ex, echo=TRUE, eval=FALSE>>=
...
\begin{figure}[htbp]
\centering
\includegraphics[trim=3cm 0.25cm 3cm 0cm,width=1\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-2Dhist.pdf
}
...
<<hist_2dmat_Ex, echo=TRUE, eval=FALSE>>=
...
\begin{figure}[htbp]
\centering
\includegraphics[trim=0cm 0.4cm 0cm 1.5cm,width=0.9\textwidth,clip]{doc/ggESDA_Jiang&Wu_20210915-2DhistMatrix.pdf
}
...
<<radar_typicalEx, echo=TRUE, eval=FALSE>>=
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{doc/radar_typical.pdf} 
...
\begin{figure}[htbp]
\centering
\includegraphics[trim=0cm 2cm 0cm 3.5cm,width=1\textwidth, clip]{doc/radar_3obs.pdf} 
...
<<radar_2obs, echo=TRUE, eval=FALSE>>=
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{doc/ggESDA_Jiang&Wu_20210915-radar.pdf} 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{doc/radar_2datasets.pdf} 
...
As far as generalization and extension are concerned, the package provides a simple way for making connections with other useful packages, so that the result of common statistical or machine learning methods on other packages may be visualized as well using \pkg{ggESDA} if it is interval-valued. In addition, we may get the classical data most of the time, so summarization between classical to interval-valued data becomes important. The following will discuss these explicitly. 
...
\begin{table}[htbp]
\centering
\caption{Fields of the built-in datasets of \pkg{ggESDA}}
\setlength{\extrarowheight}{6pt}
\resizebox{\textwidth}{!}{
  \begin{tabular}{ll}
  \toprule
  \multicolumn{2}{c}{Built-In interval-valued data set} \\
  \hline
...
=======
>>>>>>> 898d21a39c4cfdfcf9ad04236fabc09f5f5b326e
>>>>>>> Jiang_start_0120
\bibliography{refs}
