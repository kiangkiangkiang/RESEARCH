...
First, assume that each object is equally likely to be observed with probability $\frac{1}{m}$($m$ is the number of concepts), and the empirical density function of $Z$ \cite{bertrand:2000} is defined as : 

\begin{equation}\label{eq:dist1}
f(\xi) = \frac{1}{m} \sum_{u:\xi \in Z(W_u)}(\frac{1}{b_u-a_u})
\end{equation}

where $\xi$ is the individual descriptions, and $u=1,2,\cdots,m$.
...
\subsubsection{Non-equidistant Histogram}
