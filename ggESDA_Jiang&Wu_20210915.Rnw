...
The typical graphical techniques of EDA include
multiple plots for univariate data. In this field, it reveals the trend, distribution, how large the data spread, etc, for a particular variable. Moreover, it's worth mentioning that there will be some advanced graphics implemented which are called the min-max plot, and center-range plot due to the characteristics of interval-valued data. They help researchers to be able to grasp the relationship between center and range, and the difference of range in data.


\subsubsection{Presentation of interval-valued data}

There are many ways to visualize the interval of data. One of the well-known techniques especially in interval-valued data is the min-max plot. As Figure \ref{fig:minmax} shown, we visualize the oils dataset for example by the function \code{ggInterval_minmax()}. 

Figure \ref{fig:minmax_local} shows the difference between each location of concept in each variable through the 45-degree line, which is a relative position of the variable itself. And represents the range by the connection between minimum and maximum. 
...
<<minmax_local, echo=FALSE, fig=TRUE, height=4, width=8>>=
plotLocal <- NULL
for(i in 1:4){
  plotLocal[[i]] <- ggInterval_minmax(oils, aes(oils[[i]], size = 2))+
    scale_color_manual(values=c("darkblue","darkred"))+
    guides(colour = F)+theme_bw()
}
ggarrange(plotlist = plotLocal, nrow = 1, ncol = 4, labels = "")
@
                }
                \\
...
<<minmax_global, echo=FALSE, fig=TRUE, height=4, width=8>>=
plotGlobal <- NULL
for(i in 1:4){
  plotGlobal[[i]] <- ggInterval_minmax(oils, aes(oils[[i]], size = 2),
                                scaleXY = "global")+
    scale_color_manual(values=c("darkblue","darkred"))+
    guides(colour = F)+theme_bw()
}
ggarrange(plotlist = plotGlobal, nrow = 1, ncol = 4, labels = "")
@
        }
        \end{tabular}
...
Another graphical technique, called index plot, is famous in conventional data as well. It is composed of minimum and maximum in interval-valued data and exhibits the concepts' behavior. Although both of the function \code{ggInterval_index()} and \code{ggInterval_indexImage()} can achieve this objective, there is still a slight difference for visualizing. 
...
\begin{figure}[h]
    ...
    \begin{tabular}{c c}
        ...
        \scalebox{.5}{
        \label{fig:index}
<<index, echo=FALSE, fig=TRUE, height=4, width=4>>=
m <- mean(facedata$AD)
...
@
        ...
        \scalebox{.5}{
        \label{fig:index_bar}
<<index_bar, echo=FALSE, fig=TRUE, height=4, width=4>>=
...
@
        ...
        \\
                ...
                \scalebox{.5}{
                \label{fig:image}
<<index_image1, echo=FALSE, fig=TRUE, height=4, width=4>>=
...
<<index_image2, echo=FALSE, fig=TRUE, height=4, width=4>>=
ggInterval_indexImage(facedata, aes(x = AD),
                            full_strip  = T)+
  coord_flip()+
  theme_bw()+
...
@
        ...
        \end{tabular}
   \caption{\label{fig:index} Index plot for the variables AD}
\end{figure}
...
\subsubsection{Characteristic of interval-valued data}

Take the mushroom dataset for example, the side-by-side boxplot and center-range plot provide a quick view of data distribution, dispersion, and are particularly useful for comparing distributions between several groups of variables. Using the function \code{ggInterval_boxplot()} and \code{ggInterval_centerRange()} will easily construct the visualization like Figure \ref{fig:box_centerRange_before}. 
...
<<box_centerRange_before, echo=FALSE, fig=TRUE, height=6, width=6>>=
set.seed(20211216)
a <- ggInterval_boxplot(mushroom, aes(Cap.Widths),
                   plotAll = T) +
  theme_classic()+
...
b<- ggInterval_centerRange(mushroom, aes(size = 1.5),
                       plotAll = T)+
...
<<box_centerRange_after, echo=FALSE, fig=TRUE, height=6, width=6>>=
set.seed(20211216)
mushroom.scale <- scale_sym_table(mushroom)
a <- ggInterval_boxplot(mushroom.scale, plotAll = T) +
  theme_classic()+
  theme(legend.position = "bottom",
        axis.text.x = element_blank())


b<- ggInterval_centerRange(mushroom.scale, aes(size = 1.5),
                       plotAll = T)+
  theme_classic()+ stat_ellipse(geom = "polygon", fill = "blue",
                                alpha = 0.05, col = "grey")
gridExtra::grid.arrange(a, b, ncol=1, nrow = 2)
...
In the same way, demonstrating the distribution of data by histogram is also one of the most common ways. Use the blood dataset to illustrate, and construct the frequency of data by Section \ref{sec:hist} for the first, which will be processed by \code{ggInterval_hist()} function and the parameter \code{method} is the key for setting whether the histogram bins are equal width. 
...
\begin{figure}[h]
    ...
    \begin{tabular}{c c}
        ...
        \scalebox{.6}{
<<hist_equal, echo=FALSE, fig=TRUE, height=6, width=6>>=
ggInterval_hist(Cardiological, plotAll = T, bins = 30) +
  theme_hc()+
...
@
        ...
        \scalebox{.6}{
<<hist_unequal, echo=FALSE, fig=TRUE, height=6, width=6>>=
...
\begin{figure}[h]
\centering
<<scaMatrix, echo=FALSE, fig=TRUE, height=6, width=8>>=
ggInterval_scaMatrix(oils) +
      geom_smooth(data = . %>% filter(isPlot),
                         method = "lm",
                         se = T,
                         alpha = 0.3,
                         level = 0.8,
...
\begin{figure}[h]
    ...
    \begin{tabular}{c c}
        ...
        \scalebox{.6}{
<<PCA_MCAR, echo=FALSE, fig=TRUE, height=6, width=6>>=
...
\begin{figure}[h]
\centering
<<2Dhist, echo=FALSE, fig=TRUE, height=6, width=9>>=
ggInterval_2Dhist(Cardiological, aes(Pulse, Diast, col = "grey30"),
                  xBins = 25,
